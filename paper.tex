...
% the metrics
For each comment, we computed a similarity and dissimilarity metrics
...
where $A$, $B$, $C$, and $D$ are some constant.\footnote{We also have tried using the `or' operator. We found that using `and' produces better result.}

% finding parameters
To find these parameters, we used a brute-force approach.
